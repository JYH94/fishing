...
export function makeMarker(pointList, setPoint, setModal, setWhichPoint) {
    var { kakao } = window;
    ...
    function displayMarker(place) {
        ...
        kakao.maps.event.addListener(marker, 'click', function () {
            ...
            infowindow.setContent(
                `<div id="infoWindow" style="padding:5px;font-size:12px;height:70px">
                    ...
                    ${place.pointAddr}
                    ${setModal ? '<div id="detailButton" style="cursor:pointer;color:blue;">상세보기</div>' : ''}
                </div>`);
            infowindow.open(map, marker);

            // 인포윈도우가 열린 후에 이벤트 핸들러를 추가합니다
            if (setModal) {
